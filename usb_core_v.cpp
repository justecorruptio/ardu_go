// VENDORED from arduino core 1.8.6 USBCore.cpp -- overrides the core.a
// copy at link time (sketch objects satisfy the symbols first, so
// the archive member is never pulled). Trimmed for flash: the CDC
// DATA plane (Serial_ API) is unused by this sketch; enumeration and
...
// vendored: TX/RX LED pulse plumbing removed (cosmetic; Arduboy keeps them off)
...
// vendored: STRING_PRODUCT removed (descriptor index 0)
...
// vendored: STRING_MANUFACTURER removed (descriptor index 0)
...
const DeviceDescriptor USB_DeviceDescriptorIAD =
	D_DEVICE(0xEF,0x02,0x01,64,USB_VID,USB_PID,0x100,0,0,0,1) /* vendored: no string descriptors */;
#else // CDC_DISABLED
...
const DeviceDescriptor USB_DeviceDescriptorIAD =
	D_DEVICE(0x00,0x00,0x00,64,USB_VID,USB_PID,0x100,0,0,0,1) /* vendored: no string descriptors */;
#endif
...
bool ClassInterfaceRequest(USBSetup& setup)
{
	...
	// vendored: PluggableUSB stripped (no modules registered)
	return false;
}
...
bool SendDescriptor(USBSetup& setup)
{
	...
	{
		// vendored: manufacturer/product/serial string descriptors removed
		// (device descriptor indices are 0, so the host never asks; language
		// kept as a safety net for hosts that request string 0 regardless)
		if (setup.wValueL == 0) {
