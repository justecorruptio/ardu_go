#!/usr/bin/env python3
# Margin trainer: best-anchored pairwise hinge with cost-proportional margins of the net's
...
TAIL, ANNEAL = 20, 15
KAPPA = float(__import__("os").environ.get("KAPPA", "20.0"))
NB, NW, NE = 15, 578, 98
...
for ep in range(1, EPOCHS + 1):
    ...
    for bi in range(0, npos, BSZ):
        ...
        # best-anchored hinge: score(best) must exceed score(j) by KAPPA*cost_j
        # (cost of best = 0 after clip-normalization within position)
        cmin = np.minimum.reduceat(Cb, starts)
        rel = Cb - np.repeat(cmin, sl)                  # cost above position's best
        ib = starts + np.argmin.reduceat(Cb, starts) if False else None
        # index of best per segment:
        besti = np.array([starts[k] + int(np.argmin(Cb[starts[k]:starts[k]+sl[k]])) for k in range(len(sl))])
        sbest = np.repeat(sc[besti], sl)
        viol = KAPPA * rel - (sbest - sc)               # >0 = margin violated
        h = (viol > 0) & (rel > 0)                      # ignore ties with best
        epl += float(np.sum(Wrow[h] * viol[h])); epw += Wb.sum()
        g = np.zeros_like(sc)
        g[h] = Wrow[h]                                  # push violator down
        # pull best up by the sum of its violators
        gb = np.add.reduceat(np.where(h, Wrow, 0.0), starts)
        for k in range(len(sl)): g[besti[k]] -= gb[k]
        g /= Wb.sum()
        gv = act.T @ g
    ...
    if ep % 10 == 0 or ep == 1:
        print(f"epoch {ep}: hinge {epl/epw:.4f} lr {lr:.3f}", flush=True)
